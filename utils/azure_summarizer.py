...
def collect_work_item_descriptions_and_hierarchy(projects, organization_url, work_item_id, pat, visited_ids=None, work_item_map=None, hierarchy=None, fetched_work_items=None):
    ...
    for project in projects:
        ...
        if data_json:
            ...
            type = data_json.get('fields', {}).get('System.WorkItemType', 'No description available')
            if type == 'Bug':
                description = "Repro Steps: " + data_json.get('fields', {}).get('Microsoft.VSTS.TCM.ReproSteps', 'No description available')
            elif type == 'Test Case':
                description = "Steps: " + data_json.get('fields', {}).get('Microsoft.VSTS.TCM.Steps', 'No description available')
            else:
                description = "Description: " + data_json.get('fields', {}).get('System.Description', 'No description available')
                acceptance_criteria = data_json.get('fields', {}).get('Microsoft.VSTS.Common.AcceptanceCriteria', '')
...
def get_work_description(epic_link : str) -> str:
    ...
    if not projects:
        { "status" : 400 , "message " :"No projects found."}
        return
